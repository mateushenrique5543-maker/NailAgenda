...
(function () {
  ...
  function sb() {
    if (!window._sb) window._sb = window.supabase.createClient(URL, KEY);
    return window._sb;
  }
  ...
  function mapService(s) {
    return { id:s.id, name:s.name, description:s.description||'', price_cents:Math.round(Number(s.price)*100), duration_min:s.duration_minutes||60, return_days:21 };
  }
  function mapAppointment(a, names) {
    ...
    return {
      ...
      service_name: names?.[a.service_id] || '',
      date: dt.toISOString().slice(0,10),
      time: dt.toTimeString().slice(0,5),
      value_cents: Math.round(Number(a.total||0)*100),
      status: FROM_DB[a.status] || a.status
    };
  }
  ...
  return { ok: true };
    ...
    async createAppointment(p) {
      ...
      const { data, error } = await sb().from('appointments').insert({
        client_id: u.user.id, client_name: c.name, service_id: p.service_id,
        start_time: start.toISOString(), end_time: end.toISOString(),
        status: 'pending', total: svc.price, notes: p.client_notes||null
      }).select().single();
      if (error) throw new Error(error.message);
      return data;
    },
    ...
    async availability(date, serviceId) {
      ...
      const { data:busy } = await sb().from('appointments')
    ...
    /* ---- Extras com resposta vazia (não usados agora) ---- */
    async clients() { const { data } = await sb().from('clients').select('*'); return data||[]; },
    ...
    async createClient() { return { temporary_password:'temp123' }; },
    async exportClients() { return ''; },
    async createPortfolio() { return { ok:true }; },
    async deletePortfolio() { return { ok:true }; },
